...
from src.stark_verifier.air.utils import public_input_to_cairo
...
ADDITIONAL_IMPORTS = [
    "src.stark_verifier.air.config.TracesConfig",
    "src.stark_verifier.air.public_input.PublicInput",
    "src.stark_verifier.air.public_input.SegmentInfo",
    "src.stark_verifier.air.public_memory.ContinuousPageHeader",
    "src.stark_verifier.air.traces.TracesDecommitment",
    "src.stark_verifier.air.traces.TracesUnsentCommitment",
    "src.stark_verifier.air.traces.TracesWitness",
    "src.stark_verifier.core.config.StarkConfig",
    "src.stark_verifier.core.fri.config.FriConfig",
    "src.stark_verifier.core.fri.fri.FriLayerWitness",
    "src.stark_verifier.core.fri.fri.FriUnsentCommitment",
    "src.stark_verifier.core.fri.fri.FriWitness",
    "src.stark_verifier.core.proof_of_work.ProofOfWorkConfig",
    "src.stark_verifier.core.proof_of_work.ProofOfWorkUnsentCommitment",
    "src.stark_verifier.core.stark.StarkProof",
    "src.stark_verifier.core.stark.StarkUnsentCommitment",
    "src.stark_verifier.core.stark.StarkWitness",
    "src.stark_verifier.core.table_commitment.TableCommitmentConfig",
    "src.stark_verifier.core.table_commitment.TableCommitmentWitness",
    "src.stark_verifier.core.table_commitment.TableDecommitment",
    "src.stark_verifier.core.vector_commitment.VectorCommitmentConfig",
    "src.stark_verifier.core.vector_commitment.VectorCommitmentWitness",
]


def extract_annotations(
    annotations: Sequence[str], prefix: str, kind: str
) -> List[int]:
    """
...
def parse_proof(identifiers: IdentifierManager, proof_json: dict):
    ...
    #     structs, public_input=public_input, z=z, alpha=alpha
    # )
    ...
    (interaction_commitment_hash,) = annotations(
        "STARK/Interaction/Commit on Trace", "Hash"
    )
    (composition_commitment_hash,) = annotations(
        "STARK/Out Of Domain Sampling/Commit on Trace", "Hash"
    )
    oods_values = annotations(
        "STARK/Out Of Domain Sampling/OODS values", "Field Element"
    )
    fri_layers_commitments = annotations("STARK/FRI/Commitment/Layer [0-9]+", "Hash")
    fri_last_layer_coefficients = annotations(
        "STARK/FRI/Commitment/Last Layer", "Field Elements"
    )
    ...
    CPU_COMPONENT_STEP = get_dynamic_or_const_value(
        dynamic_params=public_input.dynamic_params, name="CPU_COMPONENT_STEP"
    )
    ...
    assert (
        len(layer_log_sizes) == n_fri_layers + 1
    )  # Inner layers, input and last layer.
    ...
    CONSTRAINT_DEGREE = get_dynamic_or_const_value(
        dynamic_params=None, name="CONSTRAINT_DEGREE"
    )
    ...
    stark_unsent_commitment = structs.StarkUnsentCommitment(
        ...
        fri=structs.FriUnsentCommitment(
            inner_layers=fri_layers_commitments,
            last_layer_coefficients=fri_last_layer_coefficients,
        ),
...
if __name__ == "__main__":
    ...
    proof = parse_proof(identifiers=identifiers, proof_json=proof_raw)
    print(proof)
