...
from src.stark_verifier.core.fri.test_utils import (
...
def test_fri(
    program: Program, structs: CairoStructProxy, fri_step: int, num_queries: int
):
    layer_size_log2 = 5
    ...
    for coset_index in sorted(queried_cosets):
        ...
        expected_next_queries.append(
            structs.FriLayerQuery(
                ...
                x_inv_value=div_mod(
                    1, pow(x, coset_size, DEFAULT_PRIME), DEFAULT_PRIME
                ),
            )
        )
    ...
    assert (
        res_range_check_ptr.segment_index
        == runner.range_check_builtin.base.segment_index
    )
    ...
    assert (
        res_next_queries
        == next_queries + len(expected_next_queries) * structs.FriLayerQuery.size
    )
